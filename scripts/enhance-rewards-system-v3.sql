-- Enhanced Rewards System with Float Management
...
CREATE TABLE IF NOT EXISTS yield_generation (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    allocation_id UUID REFERENCES float_allocations(id),
...
-- Initialize Puff Vault with default values
INSERT INTO puff_vault_balances (total_balance, merchant_contributions, rewards_pool_balance, float_balance)
VALUES (1250000.00, 125000.00, 50000.00, 1075000.00)
ON CONFLICT DO NOTHING;

-- Initialize default float allocations
INSERT INTO float_allocations (allocation_type, allocated_amount, allocation_percentage, target_apy, current_apy) VALUES
...
INSERT INTO rewards_catalog (merchant_id, name, description, category, points_cost, value_dollars, image_url) VALUES
((SELECT id FROM merchant_profiles LIMIT 1), '10% Off Next Order', 'Get 10% discount on your next cannabis purchase', 'discount', 200, 10.00, '/placeholder.svg?height=200&width=300'),
((SELECT id FROM merchant_profiles LIMIT 1), 'Free Delivery', 'Free delivery on orders over $50', 'discount', 150, 8.00, '/placeholder.svg?height=200&width=300'),
((SELECT id FROM merchant_profiles LIMIT 1), 'Cannabis Education Workshop', 'Join our monthly cannabis education session', 'event', 500, 25.00, '/placeholder.svg?height=200&width=300'),
((SELECT id FROM merchant_profiles LIMIT 1), 'Premium Grinder', 'High-quality 4-piece aluminum grinder', 'product', 800, 40.00, '/placeholder.svg?height=200&width=300'),
((SELECT id FROM merchant_profiles LIMIT 1), 'PuffPass T-Shirt', 'Official PuffPass branded merchandise', 'merch', 300, 15.00, '/placeholder.svg?height=200&width=300')
ON CONFLICT DO NOTHING;

-- Sample merchant contributions
INSERT INTO merchant_fee_contributions (merchant_id, fee_type, fee_amount, contribution_to_vault, contribution_to_rewards) VALUES
((SELECT id FROM merchant_profiles LIMIT 1), 'transaction_fee', 25.00, 20.00, 5.00),
((SELECT id FROM merchant_profiles LIMIT 1), 'withdrawal_fee', 37.50, 30.00, 7.50),
((SELECT id FROM merchant_profiles LIMIT 1), 'processing_fee', 15.00, 12.00, 3.00)
ON CONFLICT DO NOTHING;
